...
function normalizeSnapshot(result: string) {
	const testDirRegex = new RegExp(testDir.replace(/([/.+^${}()|[\]\\])/g, '\\$1'), 'g');
	...
	return result
		...
		.replace(/generated by Svelte v\d\.\d+\.\d+/g, 'generated by Svelte vXXX') // compiler version comment
		.replace(/"total": *\d+\.\d+/g, '"total":0.123456789') // svelte compile stats
		.replace(testDirRegex, '<testDir>');
}
describe('raw', () => {
