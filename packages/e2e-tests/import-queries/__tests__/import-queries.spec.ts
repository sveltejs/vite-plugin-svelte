...
function normalizeSnapshot(result: string) {
	...
	return result
		.replace(`/* ${normalizePath(testDir)}`, `/* ${path.basename(testDir)}`) // remove abs path from compiler comment
		.replace(/generated by Svelte v\d\.\d+\.\d+(?:-[a-z]+\.\d+)?/g, 'generated by Svelte vXXX') // remove svelte version from compiler comment
