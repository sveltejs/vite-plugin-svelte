<script>
	...
	const icon = `data:image/svg+xml;base64,${btoa(
		`
<svg xmlns="http://www.w3.org/2000f/svg" width="21" height="25" viewBox="0 0 107 128">
...
{#if show_toggle}
	<button
		...
		class:dragging
		style={`
			background-image: var(--svelte-inspector-icon); 
...
<style>
	...
	#svelte-inspector-toggle {
		...
		/* The clamps are necessary to keep the toggle in the viewport no matter what */
		left: clamp(
			var(--svelte-inspector-toggle-inset),
			var(--svelte-inspector-toggle-left),
			calc(100vw - var(--svelte-inspector-toggle-inset))
		);
		top: clamp(
			var(--svelte-inspector-toggle-inset),
			var(--svelte-inspector-toggle-top),
			calc(100vh - var(--svelte-inspector-toggle-inset))
		);
	}
